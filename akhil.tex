...
\begin{figure}[H]
 ...
 \hfill
	\begin{subfigure}[t]{0.48\textwidth}
    	...
    	\end{subfigure}
\caption{Overlapping clusters produced by IKC(10)+AOC.  Overlapping clusters were generated from CEN data using IKC\_k10 followed by either AOC\_m (left) or AOC\_k (right). Edges are drawn between clusters based on the Jaccard Coefficient for node overlap and are visible if the JC exceeds the median value. Cluster numbers in both panels correspond to cluster numbers from the input IKC clustering.}
