...
\centering
 ...
 \hfill
	\begin{subfigure}[t]{0.48\textwidth}
    	...
    	\end{subfigure}
\captionsetup{width=0.9\textwidth}	
\caption{\textcolor{blue}{The caption needs to change.}
Highly cited nodes and cluster assignments.
The publications (i.e., nodes in the graph) are partitioned into 
five groups based on their in-network total degrees, with group 1 the nodes with the smallest
total degree and group 5 the nodes with the largest total degree (more than 100,000).
The left subfigure shows the distribution of numbers of clusters each publication belongs to
for each group, and 
the right subfigure shows cluster count by tier\_1 membership for each group;
each subfigure presents these results for both AOC\_m and AOC\_k.
\textcolor{blue}{The group information is incorrect; George, please fix. ALso, please make the text
in the subfigures larger, because unreadable.}
gp 1: 0-9 citations, gp 2: 10-99 citations, gp 3: 100-999 citations, gp 4: 1000-9999 citations, gp 5: 10000 citations. }
\label{fig:fig3}
\end{figure}
