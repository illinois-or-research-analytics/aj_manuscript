...
\section{Introduction} The problem of identifying and describing research communities in the modern scientific enterprise motivates the work presented in this article. In addition to discovery, an improved quantitative understanding of the organization of the scientific enterprise also supports planning, analysis, and evaluation in the governance of science from the perspective of policy makers, funders, research institutions and the evaluation community. 

 Consequently, we are interested in scalable methods for the identification and characterization of research communities. Since research communities are expressions of scientific specialization \citep{Chubin1976,Morris2009} that can span traditional disciplinary definitions \citep{Waltman2012}, an opportunity exists to study them beyond the large scale aggregations of disciplines-- at scale and in the modern research enterprise. An additional motivation is that, on account of expansion, increased funding, globalization, and industrial-scale reorganization, the community structure of science is unlikely to mirror earlier observations \citep{Kuhn1970,Price1966,small_specialties_1979,crane1972invisible}. 

One window through which research communities can be examined is citations in the scientific literature. This approach is built on the assumption that members of a research community are more likely to cite each other's work than the work of others. Thus, in the graph (or network) of scientific literature, citation dense areas suggest the existence of communities analogous to the `research fronts' described in \cite{Price1965}.  Discovering such communities can then be  framed as a community finding problem where a community is defined as a set of vertices in a graph that exhibit greater similarity to each other than vertices outside such a community. This similarity can be defined inss ways- in our case, we measure citation density. A considerable literature exists on community finding in graphs, and a variety of community finding or clustering approaches have been used in scientometrics~\citep{Newman2006,Fortunato2009,Boyack2010,Boyack2019,Traag2019,Ahlgren2020,Chandrasekharan2021,Wedell2022}.We stress that citation density is not a convincing argument for the existence of a community. However, community finding techniques for graphs are valuable in being able to efficiently search large datasets for putative communities that can be qualitatively examined by domain experts using complementary techniques. Finally, the authors extracted from the publications in a community constitute the research community [CITE Price, Mullins, Crane, Small, Chandrasekharan, Wedell etc.)

Beyond community detection, we are especially interested in substructure (structure within communities), as it reflects roles and dynamics among members. \cite{Price1966}, in their study of the oxidative phosphorylation community, reported center–periphery substructure \citep{Breiger2014}; a small core of influential researchers and a much larger transient population. Core–periphery or center–periphery patterns have been reported in other networks using different techniques, such as block modeling and k-core decomposition arguing for generality in their occurrence [CITE Rombach, Haveman, Gallagher, Sengupta, others].

We recently reported a recursive algorithmic approach based on the k-core property that is implemented in a scalable pipeline for discovery of communities with core-periphery structure \citep{Wedell2022}. In this article, we acknowledge the limitations of disjoint clustering in the context of scientometric analyses. Specifically, that a scientific article can reasonably be assigned to more than one community. This argument is especially relevant to articles that describe widely used methods [cite Van Noorden] but also true of articles that report discovery. This limitation has been addressed by others [cite Evans \& Lambiotte, Havemann] through line graph transformation approaches but these suffer from a lack of scalability. 

In this article, we combine our interests in core-periphery structures and overlapping clusters by presenting Accommodating Overlapping Clusters (AOC), a scalable method that is designed to allow multiple assignments of nodes across the cores core-periphery community allows members of cores to be assigned to multiple cores. 

\section{Materials and Methods}

\subsection{Data} In the present study, we removed retracted and high-referencing articles from a previously generated citation network \citep{Wedell2022} inclusive of the exosome, and more generally the extracellular vesicle, literature \citep{harding1983,raposo2021}. Retracted articles were identified from a database provided by Retraction Watch and matched to the nodes in our network using digital object identifiers (DOIs). Any article that had 250 or more references was also excluded from the network. The resultant network comprised 13,989,436 nodes and 92,051,051 edges (the original network consisted of 14,695,475 nodes and 99,663,372 edges) and, in this article, is referred to as the Curated Exosome Network (CEN).
