...
To ask whether node degree is associated with the number of clusters assigned to, we examined all nodes in the network and compared their total in-graph degree to the number of clusters they were assigned, either in AOC\_m or AOC\_k treatment of IKC\_k10 clustering of the CEN (Figure~\ref{fig:fig3}). 
...
Bearing in mind that these nodes are the ones that were added as core members to clusters, this is a further indication that even membership in group 2 reflects higher than typical  citation counts.
...
On the other hand, there are highly cited publications that  are assigned to a small number of communities (even just one community), and publications that are not very highly cited (e.g., members of groups 1 and 2) that are in 9 communities. 
...
\textcolor{blue}{George, here is whee I think we could give some examples.
\begin{itemize}
\item Publications in Group 4 that are only in one community
\item Publications in Group 1 that are in 5 communities (which is  the median for Group 4)
\item Publications in Group 2 that are in 5--9 communities (above the median for Group 4)
\end{itemize}
}
