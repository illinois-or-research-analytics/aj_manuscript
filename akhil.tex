...
\begin{figure}[H]
 ...
 \hfill
	\begin{subfigure}[t]{0.48\textwidth}
    	...
    	\end{subfigure}
\captionsetup{width=0.9\textwidth}	
\caption{Highly cited nodes and cluster assignments. For AOC\_m (left panel) and AOC\_k (right panel) treatment of IKC\_k10 output, the in-graph citation count of nodes is plotted against the number of comunities a node is assigned to. Nodes are grouped into panels according to their in-graph citation count. gp1: 0-9 citations, gp2: 10-99 citations, gp3: 100-999 citations, gp4: 1000-9999 citations, gp5: $>$10,000 citations. The number of data points in each group is shown on the right hand side.}
\label{fig:fig3}
\end{figure}

To ask whether node degree is associated with the number of clusters assigned to, we examined all nodes in the network and compared their in-graph citation count to the number of clusters they were assigned to, either in AOC\_m or AOC\_k treatment of IKC\_k10 clustering of the CEN (Figure~\ref{fig:fig3}). It is evident that high in-graph citation count is necessary but not sufficient for multiple cluster assignments since all the nodes assigned to 10 or more communities have at least 10,000 in-graph citations. However, these are a relatively small number and represent \emph{George add some notes about what kinds of papers these are}. More interesting, however, are the highly cited nodes in a small number of clusters.We also note that some nodes of high-degree are singletons that are not assigned to any community in IKC because they do not meet the $k$ criterion. \emph{what does this mean? we should discuss}.
