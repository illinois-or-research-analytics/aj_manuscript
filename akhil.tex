...
As we note, in the preceding sections, AOC is a meta-method for overlapping communities that takes the output of the IKC method, a set of candidate nodes for consideration of membership in multiple communities, and a parameter that defines the criterion for membership. Since we assert that some scientific articles could formally be assigned to more than one community, we first tested AOC using, as candidates, the set of nodes in the cores extracted from the CEN network by IKC. 

AOC was run in either of two modes. One in which the parameter k was set to 10 (AOC\_k) and the other in which the existing minimum core degree (MCD) was enforced (AOC\_m). AOC\_k is more permissive since it enforces a lower bound for core degree across the clustering, whereas AOC\_m requires that any node added must meet the minimum core degree within a community in the clustering. Figure~\ref{fig:fig1} shows the distribution of cluster sizes generated by AOC relative to the core sizes from the IKC run at various values for k. From previous experience \citep{Wedell2022} and calibration experiments on the CEN network (Supplementary Data), we use IKC with its \emph{k} parameter set to 10 (IKC\_k10) for the rest of this study. At this setting, 128 cores are discovered with minimum core degree varying from 10 to 52. [Provide some more descriptive stats about CEN-- (IKC\_k10).] 
