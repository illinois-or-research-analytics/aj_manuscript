...
We recently reported Iterative K-core Clustering or IKC \citep{Wedell2022}, a recursive algorithmic approach based on the k-core property \citep{Giatsidis2011,malliaros2019}. Enumerating k-cores helps identify densely connected parts of a graph.\emph{We need another sentence about the usefulness of k-core based approaches. Giatsidis comments on ``robustness to degeneracy". Leskovec comments that ``the distribution of k-cores shows how quickly the network shrinks as one moves towards its core."} IKC recursively extracts k-cores from a graph beginning with the most dense  core. 
...
\emph{Marker nodes}. To identify exosome-relevant publications and communities, we re-used a set of marker nodes described in  \citep{Wedell2022}. These are the cited references in 12 different recent review articles on exosomes and extracellular vesicles and consist of 1218 unique nodes. Of these, 1021 are present in the CEN. 
...
break up the large clusters and add periphery nodes. This study is focused on core stucture.
...
\begin{figure}[H]
 ...
 \hfill
	\begin{subfigure}[t]{0.48\textwidth}
    	...
    	\end{subfigure}
\captionsetup{width=0.9\textwidth}	
\caption{Highly cited nodes and cluster assignments. For AOC\_m (left panel) and AOC\_k (right panel) treatment of IKC\_k10 output, the in-graph citation count of nodes is plotted against the number of comunities a node is assigned to. Nodes are grouped into panels according to their in-graph citation count. gp1: 0-9 citations, gp2: 10-99 citations, gp3: 100-999 citations, gp4: 1000-9999 citations, gp5: $\geq$10,000 citations. The number of data points in each group is shown on the right hand side.
\textcolor{blue}{George, check the counts per set, because they are equal on the left and right.}
}
\label{fig:fig3}

\end{figure}

To ask whether node degree is associated with the number of clusters assigned to, we examined all nodes in the network and compared their in-graph citation count to the number of clusters they were assigned to, either in AOC\_m or AOC\_k treatment of IKC\_k10 clustering of the CEN (Figure~\ref{fig:fig3}). 
The first comparison is between clusters produced by AOC\_m (left) and
AOC\_k (right), which shows that in general
the number of clusters that a node is in is larger for AOC\_k than
for AOC\_m, which is as expected.
We also see that Tier 1 count increases for AOC\_k compared to
AOC\_m; since cluster size  increases for AOC\_k compared to AOC\_m and
Tier 1 contains the top 10\% of the nodes in each cluster,
...
The publictions ae separated  into 5 groups based on 
...
The first striking observation is that the number of clusters
each publication belongs in is clearly impacted by the
citation count.
Thus, the largest number of communities for publications in
groups 1 and 2 is 7, group 3 publications appear in at most 9
clusters, group 4 publications appear in up to 13 clusters, 
but group 5 publications appear in up to 25 clusters.
Thus, all the nodes assigned to 14 or more communities have at least 
10,000 in-graph citations.
In other words, ultra-high in-graph citation count is necessary  for a publication to appear
in many communities.
However, the median number of communities for cluster memberships 
in group 5 is \textcolor{blue}{only KKK, indicating clearly that
 an ultra-large citation count seems to be a necessary but
not sufficient property for
membership in many conditions.
Furthermore, 
there are publications from groups 2--4 that also appear
