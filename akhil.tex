...
The SABPQ protocol  \citep{Wedell2022} used to construct the CEN is inclusive by design. It captures nodes that are proximal, by citation, to a seed set identified through a lexical search \citep{Wedell2022}.While the distribution of resultant clusters is of interest at a landscape level, we are specifically interested in those that are rich in content relating to extracellular vesicle (EV) biology. Therefore, we used 1,021 marker nodes (Materials and Methods) to identify communities reflecting EV content. Marker nodes were located in 40 of 128 IKC clusters with [insert right number] clusters accounting for [lkjsdlkjsdf].
