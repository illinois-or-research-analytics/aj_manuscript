...
\section{Introduction} The problem of identifying and characterizing research communities in the modern scientific enterprise motivates the work presented in this article. Research communities represent scientific specialization \citep{Chubin1976,Morris2009} and evolve in response to new paradigms and incentives from funders in a background of increasing globalization, and industrial-scale reorganization. Consequently, we are interested in scalable methods for the identification and characterization of research communities, particularly recently emerging ones. 
...
Figure~\ref{fig:fig1} shows the distribution of cluster sizes generated by AOC relative to the core sizes from the IKC run at various values for k. 

From previous experience \citep{Wedell2022} and calibration experiments on the CEN network (Supplementary Data), we use IKC with its \emph{k} parameter set to 10 (IKC\_k10) for the rest of this study. At this setting, 128 cores are discovered with minimum core degree varying from 10 to 52. [Provide some more descriptive stats about CEN-- (IKC\_k10).] 
