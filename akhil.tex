...
\begin{figure}[H]
 ...
 \hfill
	\begin{subfigure}[t]{0.48\textwidth}
    	...
    	\end{subfigure}
\caption{Comparison of cluster sizes between disjoint (IKC)  and overlapping (AOC) clusters; cluster size selectively increases after enrichment through AOC. Clusters were generated from CEN data by IKC using values of k ranging from 10-50. These clusters were then enriched through the AOC process enforcing either \emph{k} (left panel) or \emph{mcd} (right panel). The input to AOC was the set of nodes in the IKC clusters. A natural log scale is used for both axes.}
\label{fig:fig1}
\end{figure}
