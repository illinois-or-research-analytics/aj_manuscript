...
While there appears to be an association between the degree of a node and the likelihood of it being assigned to multiple clusters, there are instances where nodes of high degree are assigned to one or two clusters. For example, five publications are found in Group 4 that are assigned to only one cluster \citep{arnon1949copper,ellman1961new,friedewald1972estimation,iijima1991helical,raymond1995genepop}. Four of these five describe methods. All five were published in or before 1995 (1949-1995), have high in-degree (12,741 to 32,927) and very low out-degree in our data (0-11). Whether this low out-degree contributed to restricted cluster assignment merits follow up and opens up the questions of breadth and dependence \citep{bu2021multidimensional} in publication communities, as well as that of data quality. This shows that high total degree is not sufficient for membership in many communities and underscores the case for mixed methods approaches.
...
We examine the effects of AOC from two complementary perspectives. A graph theoretic one, which is designed for generalizability, and a contextual perspective, in which we interpret findings relative to a field of interest. The first perspective is considered in the preceding sections. In the second case, we are studying communities in the field of extracellular vesicle research. Here, we address context in two ways. We use, as input to IKC, a citation network (CEN) that is enriched in the recent extracellular vesicle literature . IKC reduces this large network to 128 cores that contain 3.8\% of the nodes in the network. From these 128 cores, we identify a subset of of interest by using a set of markers derived from the cited references of recent reviews of the extracellular vesicle field that were authored by different researchers. Under the assumption that cores enriched in marker nodes are relevant to extracellular vesicle research, we further reduce the data under consideration to those cores. We now assess how AOC impacts marker node concentration.

Of 128 cores, 31 cores contained between 0.1 \% and 94.6\% of the markers. The count of cores with non zero marker counts varied between treatments and consisted of 17 (IKC), 20(AOC\_m), and 31 (AOC\_k) respectively. Cluster 3,4, and 25 are notable in accounting for 87.5\% of 1021 markers after IKC clustering. After AOC\_m treatment of IKC clusters, clusters 3,4 and 25 contained 

After AOC\_m treatment of IKC clusters, cluster 4 contained 30.4\%, 42.5\%, and 93.1\% of all markers. After AOC\_k treatment of IKC clusters, clusters 1,3,4, and 25 contained 20.7\%, 69.5\%, 71.8\%, and 94.6\% of all markers respectively (Figure~\ref{fig:marker-node-concentration}). These data also suggest that the recursive approach of IKC results in markers being segregated by disjoint clustering that can be post-processed by AOC treatment. AOC\_m and AOC\_k offer relatively stringent and permissive options respectively. From the perspective of EV biology, clusters 3,4, and 25 are of obvious interest and offer a significant reduction in the amount of information to be studied qualitatively.  We believe that AOC\_m is the better choice since it is a local constraint that preserves the MCD of each core, whereas AOC\_k enforces the value of $k$ across all the cores. AOC\_k may therefore add weakly linked nodes to the k-core. This may be of interest to some researchers, therefore, we allow both options. 
...
%:
Whereas the use of marker nodes is one approach to identify clusters of relevance, there is overlap by non-marker nodes. Accordingly, we examined overlap between clusters after AOC\_m or AOC\_k treatment of IKC clusters. Weighted edges were drawn between clusters based on the Jaccard Coefficient (ratio of intersection/union) for overlap. A threshold of the median Jaccard Coefficient from all values was set to permit an edge. The data show that IKC + AOC cores form subgraphs that contain fewer clusters in the case of AOC\_m and more in the case of AOC\_k. Interestingly, of the clusters that were rich in marker nodes, clusters 3 and 4 are found in one subgraph and cluster 25 in a second. This, for both AOC\_m and AOC\_k. \textcolor{red}{We need to discuss. It's cool but...}
