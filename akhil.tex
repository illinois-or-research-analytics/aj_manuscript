...
\abstract{
Community detection assists the understanding of complex networks. Unsurprisingly, a rich literature originating from several fields exists on the development and application of community detection. Within this area, we are focused on the problem of community detection in the very large network of scientific literature-- for the purpose of identifying research communities at different scales.  A variety of clustering methods have been developed for community detection However, these largely adopt a disjoint clustering approach where a node is only assigned to one community or cluster. This strict requirement limits the ability to inclusively describe communities. For example, a widely used method could reasonably be assigned to many communities. Whereas, we have previously described a scalable and modular pipeline that discovers disjoint communities, we now present an overlapping community finding approach that builds upon this approach. We report comparative findings from this new approach on a network of over 13 million nodes that captures recent research in the very rapidly growing field of extracellular vesicles in biology.}
