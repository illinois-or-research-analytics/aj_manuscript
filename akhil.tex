...
One approach through which research communities can be identified is analyzing citation patterns in the scientific literature. The underlying assumption is that members of a research community are more likely to cite each other's work than the work from outside their community.  
Drawing upon the rich literature from graph theory, this question can then be framed as a community finding problem where a community is defined as a set of vertices in a graph that exhibit stronger connectivity to each other than to vertices outside such a community. 
Thus, in the graph (or network) defined by scientific literature, citation-dense areas suggest the existence of communities. Finally, the authors extracted from the community of publications are assumed to form a research community [CITE Price, Mullins, Crane, Small, Chandrasekharan, Wedell etc.]. We stress that citation density alone does not make a convincing argument for the existence of a community. However, community finding techniques for graphs are valuable in being able to efficiently search large datasets for communities that can be qualitatively examined by domain experts using complementary techniques. 
...
A basic limitation to IKC is that  it produces disjoint clusters. This is a limitation,  since some articles could reasonably be assigned to more than one community. This argument is especially relevant to articles that describe widely used methods but is conceivably also relevant to articles reporting discovery that are influential in more than community.   
However, this is a common limitation for nearly all clustering methods, and the need for methods that can produce meaningful overlapping clusters is well appreciated (cite).

Several methods have been proposed that can produce overlapping clusters cite relevant papers]. 
One of the well known approaches produces overlapping clusters  in a two-step procedure where the input graph is first transformed into a line graph (whose nodes represent edges in the original graph), and then the line graph is clustered  \citep{Harary1960}, thus defining overlapping clusters  in the input graph. Approaches such as these have been successfully used by others \citep{Evans2009,Havemann2021} on citation data. 
 ...
 To address the disjoint cluster problem in IKC, we present Allowing Overlapping Clusters (AOC), a scalable meta-method that takes the output of IKC and makes multiple community assignments from a list of candidate nodes. We present results from AOC applied to the cores (centers) generated by the IKC method, and discuss the results and discovery made from them.
 
\section{Materials and Methods}

\subsection{Data} We previously generated a citation network \citep{Wedell2022} inclusive of the exosome \citep{harding1983}, and more generally the extracellular vesicle, literature \citep{raposo2021}. The network was constructed in mid-2021 from the Dimensions database \citep{hook2018dimensions} in the Google cloud. For the present study, we curated this network to remove retractions , which were identified from a database kindly provided by Retraction Watch and matched to nodes in the network using digital object identifiers (DOIs). Any article with 250 references or more was also removed. 
\textcolor{blue}{George, I think it would be good to explain why high-referencing articles are removed, and maybe even why retracted articles are rmoved.}
Whereas the original network consisted of 14,695,475 nodes and 99,663,372 edges, the network resultant from removing retracted and high-referencing articles comprised 13,989,436 nodes and 92,051,051 edges; we refer to this network as   the Curated Exosome Network (CEN).

\subsection{Methods} We have previously described a clustering pipeline we refer to as  the Iterative K-core Clustering (IKC) in \cite{Wedell2022}, motivated by the graph-theoretic concept of k-cores \citep{Giatsidis2011,malliaros2019}. This scalable and tunable method takes as input two parameters $k$ and $p$ with $k > p \geq 1$, and computes a clustering of the given network $N$ into disjoint cluster where each cluster has a ``center" component and a ``periphery". This clustering is designed to satisfy several criteria: (1) each center of each cluster is connected,  has positive modularity, and each node in the center  is adjacent to at least $k$ other nodes in the center, and (ii) every node in the periphery of a cluster is adjacent to at least $p$ center nodes in the cluster.  Thus, center membership requires a greater degree of connectivity to the other center nodes than periphery membership.
...
The Accommodating Overlapping Clusters (AOC) method, presented herein, builds on the first step of this pipeline (the IKC step itself). To run AOC, the user specifies two additional parameters:  
...
after AOC treatment. Approximately 74\% and 87\% of the 128 cores increase in size with AOC\_m and AOC\_k treatment respectively. 
 We conclude that AOC treatment of IKC clustering results in an increase in cluster sizes that is is inversely related to the value of \emph{k} used in IKC and is more pronounced with AOC\_k than with AOC\_m. 

From these data (Figure~\ref{fig:fig1}) we opted to use IKC with  $k=10$  (IKC\_k10) to generate the input clustering to AOC in all data shown in the rest of this study. At this setting, 128 cores are discovered in the CEN network that range in size from 14 to 214,877, with a median core size of 79, and minimum core degree (MCD) varying from 10-53 with median MCD=16. % derived from k_10_totaldegree_1percent_original_cluster_stats.csv in /shared/aj_manuscript_data/experiment_1/. 
...
The single point noted for IKC10\_o indicates that it produced only one non-singleton cluster.. }
\label{fig:fig2}
\end{figure}
