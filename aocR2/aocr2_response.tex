...
These two properties of the IKC clusters are essentially the definition of what this algorithm considers  a minimum standard for being a ``valid" community.  (We note that this is similar to text about the Constant Potts Model which says that any cluster whose density is above the parameter $\gamma$ should be considered a community, and similar text about modularity in which the statement we see a statement in Fortunato and Barth\'el\'emy (PNAS) that any cluster with positive modularity should be considered valid. We are paraphrasing these articles, of course; the word ``valid may not appear as such, but the sense is the same.) Thus, for IKC, given the parameter $k$, any cluster with these two properties is valid.

Yet the specific set of clusters that AOC returns is in some sense an artefact of the order in which nodes are processed and is also a consequence of the restriction of each node belonging to just one cluster. If instead we allow ourselves to consider whether other nodes would be considered valid members of the clusters produced by IKC, we would ask very simply whether the addition of a given node to a particular cluster would maintain these properties; this is AOC\_k.  A more conservative expansion would say that we would not add a node to a cluster if it reduced the MCD score of the cluster (i.e., if it reduced the minimum degree within the cluster); this is AOC\_{MCD}. 
...
Thus, the expansion using AOC is not meant to ``improve accuracy" in the sense of being reflective of some true structure, but it is instead providing a fuller exploration of the ``meaning" of the clusters we obtained in the IKC portion of the IKC+AOC pipeline.  Having said this, we do feel that the AOC\_{MCD} version if closer to reflecting the true nature of a cluster. Thus, if a cluster expands under AOC\_{MCD}  to include many nodes, then we would argue that the expanded version  is a better representation of what that community means than the original version produced by IKC. 
