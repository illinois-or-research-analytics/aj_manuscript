...
\textcolor{blue}{so that the input to AOC is a set of disjoint clusters produced by the first step of the IKC pipeline. These clusters will be enlarged by AOC,  producing expanded versions, using a simple technique that we now describe.}
To run AOC, the user specifies two additional parameters:  the set of nodes that are being considered for membership in \textcolor{blue}{the expanded} clusters and the criterion for membership.  

The two criteria for \textcolor{blue}{allowing node $v$ to be added 
to the expansion of cluster $C$ (where $C$ is in the input clustering) 
we consider are: (i) $v$ must have at least $MCD(C)$ neighbors in  $C$, 
...
\textcolor{blue}{We also require that the addition of $v$ to the current expansion of $C$ produce a cluster that has positive modularity.
This last criterion ensures that the final expanded cluster that is produced will also have positive modularity.}
	
Thus, whether running   AOC\_m or AOC\_k on the IKC clustering, both pipelines produce a set of  \textcolor{blue}{expanded clusters (compared to the input clustering)} that are potentially overlapping,  each of which has positive modularity and where every node in every cluster has at least $k$ other nodes in the cluster.  
\textcolor{blue}{If desired, these clusters (which constitute cores) could be expanded to include peripheral nodes (i.e., step 3 in the IKC pipeline).
Moreover, the input clusters could be produced in variants of the IKC pipeline (e.g., including the optional second step, which breaks up the large clusters).}

	\textcolor{blue}{George, I don't think the next paragraph is needed, because it is repeating what is above. Alternatively, we can start this off saying ``In summary"...}
Thus, the input to AOC is a clustering of a network into disjoint clusters, a value for $k$,  the criterion it uses (either AOC\_m or AOC\_k), and the list of candidate nodes. If this clustering is produced by IKC, by construction the clustering will be $km$-valid. The list of candidate nodes (the first of the two additional parameters) can be constructed from node-network characteristics such cluster membership,  total degree, in-degree,  or out-degree,  or some other basis for candidate selection such as publication venue or funding sources.
Once candidate nodes are chosen, AOC constructs overlapping clusters by adding all candidate nodes to all clusters that meet  the selected inclusion criterion (AOC\_m or AOC\_k) and ensures positive modularity is maintained with each added node.  
